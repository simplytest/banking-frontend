...
describe("Übung 7 - Banking Workflow", () =>
{
    ...
    beforeEach(() =>
    {
        ...
        cy.fixture("loginData").then((loginData) =>
        {
            ...
            cy.intercept("POST", "**/api/contracts/login/00025", {
                statusCode: 200,
                body: {
    ...
    it("Unhappy Path: Geld empfangen und überweisen mit ungültiger IBAN", () =>
    {
        cy.fixture("contracts").then((contracts) =>
        {
            ...
            {
                cy.intercept("GET", `**/api/accounts/1/receive?amount=${transactionData.receiveMoney.amount}`, {
                    statusCode: 200,
                    body: {
            ...
            {
                expect(interception.response.statusCode).to.equal(200);
                expect(interception.response.body.customer.data.firstName).to.equal("Art");
            });
            ...
            cy.fixture("transactionData").then((transactionData) =>
            {
                cy.intercept("POST", "**/api/accounts/1/send", {
                    statusCode: 400,
                    body: {
            ...
            cy.get("input[data-testid='send_amount']").click().type("100");
    ...
    it.skip("Festgeld Konto erstellen und Geld transferieren2", () =>
    {
        ...
        cy.fixture("contracts").then((contracts) =>
        {
            cy.fixture("transactionData").then((transactionData) =>
            {
                ...
                cy.intercept("POST", "**/api/contracts/accounts/FixedRateAccount", {
                    statusCode: 201,
                ...
                cy.intercept("POST", "**/api/accounts/1/transfer", {
                    statusCode: 200,
                    body: {
                ...
                cy.get("input[data-testid=\"amount_input\"]").click().type("1000");
